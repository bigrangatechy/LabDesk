//! Forgejo REST API v1 client (`Authorization: token …`).
...
pub fn play_job(
    ...
    Err(LabDeskError::App(
        ErrorInfo::new("LD-API-JOB-001", "Failed to run CI job.").with_detail(
            "Forgejo Actions does not support playing manual jobs from LabDesk.",
        ),
    ))
}
...
pub fn list_merge_request_notes(
    ...
    // Forgejo returns the full comment list in one shot; page>1 would duplicate.
    if page > 1 {
        return Ok(vec![]);
    }
    let (owner, repo) = resolve_owner_repo(base_url, pat, ssl_mode, project_id, path_hint)?;
